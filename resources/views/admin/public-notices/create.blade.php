...
@section('page-title', 'Novo Edital')

@section('content')
<div class="max-w-4xl">
    <div class="bg-white rounded-lg shadow">
        <div class="p-4 sm:p-6 border-b border-gray-200 bg-gradient-to-r from-primary/5 to-secondary/5">
            <h3 class="text-lg font-semibold text-gray-900">Cadastrar Novo Edital</h3>
            <p class="text-sm text-gray-600 mt-1">Preencha os campos abaixo para cadastrar um novo edital</p>
        </div>

        <form action="{{ route('admin.public-notices.store') }}" method="POST" enctype="multipart/form-data" class="p-4 sm:p-6 space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input 
                        ...
                        placeholder="Ex: CAPES, CNPq, FAPESB"
                        required
                    >
                    @error('institution')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                ...
                <div>
                    ...
                    >
                        @foreach($statuses as $key => $label)
                            <option value="{{ $key }}" {{ old('status', 'aberto') == $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                    @error('status')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            ...
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500">Breve descrição do edital (máximo 1000 caracteres)</p>
                @error('summary')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500">Informações detalhadas sobre o edital</p>
                @error('content')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
