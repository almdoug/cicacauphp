...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
        <a href="{{ route('admin.pages.edit', 'home') }}" class="block bg-white rounded-lg shadow hover:shadow-lg transition-shadow p-4 sm:p-6 group">
            <div class="flex items-center gap-3 sm:gap-4">
                <div class="flex items-center justify-center w-10 h-10 sm:w-12 sm:h-12 bg-primary bg-opacity-10 rounded-lg group-hover:bg-opacity-20 transition-colors flex-shrink-0">
                ...
                <div class="min-w-0">
                    <h4 class="font-semibold text-gray-900 text-sm sm:text-base">Editar Página Inicial</h4>
                    <p class="text-xs sm:text-sm text-gray-600">Alterar textos e conteúdos</p>
                </div>
            </div>
        </a>
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        <div class="p-4 sm:p-6 border-b border-gray-200">
            <h3 class="text-base sm:text-lg font-semibold text-gray-900">Páginas Disponíveis</h3>
        </div>
        <div class="divide-y divide-gray-200">
            @foreach($pages as $key => $name)
                <div class="p-4 sm:p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-3 hover:bg-gray-50 transition-colors">
                    <div class="min-w-0">
                        <h4 class="font-medium text-gray-900 text-sm sm:text-base">{{ $name }}</h4>
                        <p class="text-xs sm:text-sm text-gray-600">{{ ucfirst($key) }}</p>
                    </div>
                    <a href="{{ route('admin.pages.edit', $key) }}" class="inline-flex items-center justify-center gap-2 bg-primary text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-opacity-90 transition-all flex-shrink-0">
                        <span>Editar</span>
