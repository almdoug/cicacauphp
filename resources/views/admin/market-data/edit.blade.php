...
@section('page-title', 'Editar Dado de Mercado')

@section('content')
<div class="max-w-4xl">
    <div class="bg-white rounded-lg shadow">
        <div class="p-4 sm:p-6 border-b border-gray-200 bg-gradient-to-r from-primary/5 to-secondary/5">
            <h3 class="text-lg font-semibold text-gray-900">Editar Dado de Mercado</h3>
            <p class="text-sm text-gray-600 mt-1">Atualize os dados de mercado</p>
        </div>

        <form action="{{ route('admin.market-data.update', $marketData) }}" method="POST" enctype="multipart/form-data" class="p-4 sm:p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <input 
                        ...
                        name="title" 
                        id="title" 
                        value="{{ old('title', $marketData->title) }}"
                ...
                <div>
                    ...
                    >
                        @foreach($categories as $key => $label)
                            <option value="{{ $key }}" {{ old('category', $marketData->category) == $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                    @error('category')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                ...
                <div>
                    ...
                    >
                        @foreach($scopes as $key => $label)
                            <option value="{{ $key }}" {{ old('scope', $marketData->scope) == $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                    @error('scope')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                ...
                <div>
                    ...
                    <input 
                        ...
                        name="region" 
                        id="region" 
                        value="{{ old('region', $marketData->region) }}"
                ...
                <div>
                    ...
                    <input 
                        ...
                        name="period" 
                        id="period" 
                        value="{{ old('period', $marketData->period) }}"
                ...
                <div>
                    ...
                    <input 
                        ...
                        name="value" 
                        id="value" 
                        value="{{ old('value', $marketData->value) }}"
                        step="0.01"
                ...
                <div>
                    ...
                    <input 
                        ...
                        name="unit" 
                        id="unit" 
                        value="{{ old('unit', $marketData->unit) }}"
                ...
                <div>
                    ...
                    <input 
                        ...
                        name="variation" 
                        id="variation" 
                        value="{{ old('variation', $marketData->variation) }}"
                        step="0.01"
                ...
                <div>
                    ...
                    <input 
                        ...
                        name="source" 
                        id="source" 
                        value="{{ old('source', $marketData->source) }}"
            ...
            <div>
                ...
                <textarea 
                    ...
                    required
                >{{ old('summary', $marketData->summary) }}</textarea>
                @error('summary')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                >{{ old('content', $marketData->content) }}</textarea>
                @error('content')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    @if($marketData->file)
                        <div class="mb-2 p-3 bg-gray-50 rounded-lg flex items-center justify-between">
                            <span class="text-sm text-gray-600">Arquivo atual: {{ basename($marketData->file) }}</span>
                            <a href="{{ $marketData->getFileUrl() }}" target="_blank" class="text-primary hover:underline text-sm">Ver</a>
                        </div>
                    @endif
                    <input 
                        type="file" 
                ...
                <div>
                    ...
                    <input 
                        ...
                        name="external_link" 
                        id="external_link" 
                        value="{{ old('external_link', $marketData->external_link) }}"
            ...
            <div>
                ...
                <input 
                    ...
                    name="published_at" 
                    id="published_at" 
                    value="{{ old('published_at', $marketData->published_at ? $marketData->published_at->format('Y-m-d\TH:i') : '') }}"
