...
@section('page-title', 'Editar Pesquisa')

@section('content')
<div class="max-w-4xl">
    <div class="bg-white rounded-lg shadow">
        <div class="p-4 sm:p-6 border-b border-gray-200 bg-gradient-to-r from-primary/5 to-secondary/5">
            <h3 class="text-lg font-semibold text-gray-900">Editar Pesquisa</h3>
            <p class="text-sm text-gray-600 mt-1">Atualize os dados da pesquisa</p>
        </div>

        <form action="{{ route('admin.researches.update', $research) }}" method="POST" enctype="multipart/form-data" class="p-4 sm:p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    >
                        @foreach($types as $key => $label)
                            <option value="{{ $key }}" {{ old('type', $research->type) == $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                    @error('type')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                ...
                <div>
                    ...
                    <input 
                        ...
                        name="institution" 
                        id="institution" 
                        value="{{ old('institution', $research->institution) }}"
            ...
            <div>
                ...
                <textarea 
                    ...
                    required
                >{{ old('summary', $research->summary) }}</textarea>
                @error('summary')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    @if($research->file)
                        <div class="mb-2 p-3 bg-gray-50 rounded-lg flex items-center justify-between">
                            <span class="text-sm text-gray-600">Arquivo atual: {{ basename($research->file) }}</span>
                            <a href="{{ $research->getFileUrl() }}" target="_blank" class="text-primary hover:underline text-sm">Ver</a>
                        </div>
                    @endif
                    <input 
                        type="file" 
                ...
                <div>
                    ...
                    <input 
                        ...
                        name="external_link" 
                        id="external_link" 
                        value="{{ old('external_link', $research->external_link) }}"
            ...
            <div>
                ...
                <input 
                    ...
                    name="published_at" 
                    id="published_at" 
                    value="{{ old('published_at', $research->published_at ? $research->published_at->format('Y-m-d\TH:i') : '') }}"
