...
@section('page-title', 'Editar Patente')

@section('content')
<div class="max-w-4xl">
    <div class="bg-white rounded-lg shadow">
        <div class="p-4 sm:p-6 border-b border-gray-200 bg-gradient-to-r from-primary/5 to-secondary/5">
            <h3 class="text-lg font-semibold text-gray-900">Editar Patente</h3>
            <p class="text-sm text-gray-600 mt-1">Atualize os dados da patente</p>
        </div>

        <form action="{{ route('admin.patents.update', $patent) }}" method="POST" enctype="multipart/form-data" class="p-4 sm:p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input 
                        ...
                        name="patent_number" 
                        id="patent_number" 
                        value="{{ old('patent_number', $patent->patent_number) }}"
                ...
                <div>
                    ...
                    >
                        @foreach($statuses as $key => $label)
                            <option value="{{ $key }}" {{ old('status', $patent->status) == $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                    @error('status')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                ...
                <div class="md:col-span-2">
                    ...
                    <input 
                        ...
                        name="inventors" 
                        id="inventors" 
                        value="{{ old('inventors', $patent->inventors) }}"
                ...
                <div>
                    ...
                    <input 
                        ...
                        name="institution" 
                        id="institution" 
                        value="{{ old('institution', $patent->institution) }}"
                ...
                <div>
                    ...
                    <input 
                        ...
                        name="filing_date" 
                        id="filing_date" 
                        value="{{ old('filing_date', $patent->filing_date?->format('Y-m-d')) }}"
                ...
                <div>
                    ...
                    <input 
                        ...
                        name="grant_date" 
                        id="grant_date" 
                        value="{{ old('grant_date', $patent->grant_date?->format('Y-m-d')) }}"
            ...
            <div>
                ...
                <textarea 
                    ...
                    required
                >{{ old('summary', $patent->summary) }}</textarea>
                @error('summary')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    @if($patent->file)
                        <div class="mb-2 p-3 bg-gray-50 rounded-lg flex items-center justify-between">
                            <span class="text-sm text-gray-600">Arquivo atual: {{ basename($patent->file) }}</span>
                            <a href="{{ $patent->getFileUrl() }}" target="_blank" class="text-primary hover:underline text-sm">Ver</a>
                        </div>
                    @endif
                    <input 
                        type="file" 
                ...
                <div>
                    ...
                    <input 
                        ...
                        name="external_link" 
                        id="external_link" 
                        value="{{ old('external_link', $patent->external_link) }}"
            ...
            <div>
                ...
                <input 
                    ...
                    name="published_at" 
                    id="published_at" 
                    value="{{ old('published_at', $patent->published_at ? $patent->published_at->format('Y-m-d\TH:i') : '') }}"
